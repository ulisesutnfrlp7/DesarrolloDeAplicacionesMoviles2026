...
export default function UserManagementScreen() {
  ...
  const [modalConfirmarCancelarEdicion, setModalConfirmarCancelarEdicion] = useState(false);
  ...
  const obtenerMensajeEliminacion = () => {
    if (!usuarioAEliminar) return "";
    if (usuarioAEliminar.rol === 'profesor') {
      return `¿Deseás eliminar al profesor "${usuarioAEliminar.nombre}"?`;
    }
    if (usuarioAEliminar.rol === 'admin') {
      return `¿Deseás eliminar al administrador "${usuarioAEliminar.nombre}"?`;
    }
    return `¿Estás seguro de que deseas eliminar al alumno "${usuarioAEliminar.nombre}"?\n\nEsta acción borrará de forma irreversible su cuenta y revocará automáticamente todas sus inscripciones activas de la plataforma.`;
  };
  ...
  try {
    // 1. Buscamos todas las inscripciones reales del alumno usando el collectionGroup que indexaste
    const inscripcionesRef = collectionGroup(db, 'inscripciones');
    const q = query(inscripcionesRef, where('alumnoId', '==', usuarioAEliminar.id));
    const querySnapshot = await getDocs(q);

    // 2. Borramos cada documento de inscripción encontrado usando su propia referencia exacta
    const promesasBorrado = querySnapshot.docs.map(docSnap => deleteDoc(docSnap.ref));
    await Promise.all(promesasBorrado);
    console.log(`Se eliminaron ${querySnapshot.size} inscripciones de la base de datos.`);

    // 3. Eliminamos al usuario de la colección 'usuarios'
    await eliminarUsuario(usuarioAEliminar);

    // 4. Actualizamos el estado local para que desaparezca de la lista en pantalla inmediatamente
    setUsuarios(prev => prev.filter(u => u.id !== usuarioAEliminar.id));

    setAlerta({
      visible: true,
      titulo: "Usuario eliminado",
      mensaje: `Se eliminó a ${usuarioAEliminar.nombre} junto con todas sus inscripciones (${querySnapshot.size}).`,
      tipo: "exito",
    });
  } catch (e: any) {
    ...
    setAlerta({
      visible: true,
      titulo: "Error",
      mensaje: e.message || "No se pudo completar la eliminación.",
      tipo: "error",
    });
  } finally {
...
const eliminarUsuario = async (usuario: Usuario) => {
  ...
  console.log(`Usuario ${usuario.id} eliminado de la colección usuarios`);
};

  // Mapas auxiliares para renderizado
  ...
  const handleEliminarMultiples = async () => {
    ...
    setEliminando(true);
    try {
      await Promise.all(inscripcionesSeleccionadas.map(id => revocarInscripcion(id)));
  ...
  const confirmarMoverAlumno = async () => {
    ...
    try {
      ...
      setAlerta({ visible: true, titulo: "Error", mensaje: e?.message || "No se pudo mover al alumno.", tipo: "error" });
    } finally {
  ...
  return (
    <View style={{ flex: 1 }}>
        ...
        <View style={styles.container}>
          ...
          <View style={styles.rolesContainer}>
            {["todos", "alumno", "profesor", "admin"].map((rolItem) => {
              const activo = filtroRol === rolItem;
              return (
                <TouchableOpacity
                  key={rolItem}
                  style={[styles.rolButton, activo && styles.rolButtonActivo]}
                  onPress={() => setFiltroRol(rolItem as Rol | "todos")}
                >
                  <Text style={activo ? styles.rolButtonTextActivo : styles.rolButtonText}>{rolItem.toUpperCase()}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
            ...
            renderItem={({ item }) => (
              <View style={styles.card}>
                ...
                <View style={styles.cardActions}>
                  ...
                  <TouchableOpacity style={styles.deleteBtn} onPress={() => setUsuarioAEliminar(item)}>
        ...
        <ScrollView style={styles.container} contentContainerStyle={styles.listContent}>
          ...
          {loadingCursadas ? (
            ...
            <View style={styles.emptyContainer}>
              ...
              <Text style={[styles.emptyText, { fontSize: 12, marginTop: 4 }]}>{errorCursadas}</Text>
            </View>
            ...
            cursadasFiltradas.map((cursada) => {
              ...
              const moduloTitulo = loadingModulos ? 'Cargando módulo...' : (modulosMap[cursada.moduloId] ?? 'Módulo no encontrado');
              const alumnosSinInscribir = usuarios.filter(u => u.rol === 'alumno' && !inscritos.some(i => i.alumnoId === u.id));
              const alumnosFiltrados = alumnosSinInscribir.filter(u => u.nombre.toLowerCase().includes(filtroTexto.toLowerCase()) || u.email.toLowerCase().includes(filtroTexto.toLowerCase()));
                ...
                <View key={cursada.id} style={styles.cursadaCard}>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setInscripcionesSeleccionadas([]);
                      setAlumnosSeleccionados([]);
                      setCursadaExpandida(cursadaExpandida?.id === cursada.id ? null : cursada);
                    }}
                    activeOpacity={0.8}
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      {cursada.tipoAcceso === "subseccion" && <Text style={styles.cursadaSubtitulo}>Sección: {cursada.seccionTitulo}</Text>}
                      <Text style={styles.cursadaSubtitulo}>Módulo: {moduloTitulo}</Text>
                    </View>
                    <Ionicons name={expandida ? 'chevron-up-outline' : 'chevron-down-outline'} size={18} color="#9CA3AF" />
                  </TouchableOpacity>
                  ...
                  {expandida && (
                    <View style={styles.cursadaPanel}>
                      ...
                      {inscripcionesSeleccionadas.length > 0 && (
                        <TouchableOpacity style={styles.eliminarBtn} onPress={() => setModalEliminarMultiples(true)}>
                          <Text style={styles.eliminarBtnText}>Eliminar ({inscripcionesSeleccionadas.length})</Text>
                        </TouchableOpacity>
                      )}

                      {loadingInscripciones ? (
                        <ActivityIndicator color="#25B471" size="small" />
                      ) : inscritos.length === 0 ? (
                        ...
                        inscritos.map((insc) => (
                          <View key={insc.id} style={styles.inscriptoRow}>
                            ...
                            <TouchableOpacity style={styles.revocarBtn} onPress={() => toggleInscripcion(insc.id)}>
                      ...
                      <Modal visible={modalAsignar} transparent animationType="slide">
                        <View style={styles.modalOverlay}>
                          <View style={styles.modalContent}>
                            ...
                            <TextInput style={styles.buscadorInput} placeholder="Buscar por nombre..." value={filtroTexto} onChangeText={setFiltroTexto} />
                            <ScrollView style={{ maxHeight: 320 }}>
                              {alumnosSeleccionados.length > 0 && (
                                <TouchableOpacity style={styles.saveBtn} onPress={handleAsignarMultiples}>
                                  <Text style={styles.saveBtnText}>Asignar ({alumnosSeleccionados.length})</Text>
                                </TouchableOpacity>
                              )}
                              {alumnosFiltrados.map(u => (
                                <View key={u.id} style={styles.alumnoPickerRow}>
                                  <Text style={{ flex: 1 }}>{u.nombre}</Text>
                                  <TouchableOpacity onPress={() => toggleAlumno(u.id)}>
                                    <Ionicons name={alumnosSeleccionados.includes(u.id) ? "checkmark-circle" : "add-circle-outline"} size={24} color="#25B471" />
                                  </TouchableOpacity>
                                </View>
                              ))}
                            </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => { setInscripcionAMover(null); setDestinoMovimiento(null); }}>
                <Ionicons name="close" size={22} color="#6B7280" />
              </TouchableOpacity>
            </View>
            <Text style={styles.moverResumen}>{alumnoAMoverNombre}</Text>
            <ScrollView style={{ maxHeight: 260 }}>
              {destinosMovimiento.map((destino) => (
                <TouchableOpacity key={destino.id} style={[styles.destinoRow, destinoMovimiento?.id === destino.id && styles.destinoRowSelected]} onPress={() => setDestinoMovimiento(destino)}>
                  <Text style={{ flex: 1 }}>{destino.titulo}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.cancelBtn} onPress={() => { setInscripcionAMover(null); setDestinoMovimiento(null); }}>
                <Text style={styles.cancelBtnText}>Cancelar</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.saveBtn} onPress={confirmarMoverAlumno} disabled={!destinoMovimiento}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity onPress={() => setModalConfirmarCancelarEdicion(true)}>
                <Ionicons name="close" size={22} color="#6B7280" />
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.rolesContainer}>
              {(['alumno', 'profesor', 'admin'] as Rol[]).map(r => (
                <TouchableOpacity key={r} style={[styles.rolBtn, rol === r && styles.rolBtnSelected]} onPress={() => setRol(r)}>
                  <Text style={[styles.rolBtnText, rol === r && styles.rolBtnTextSelected]}>{r}</Text>
                </TouchableOpacity>
              ))}
            </View>

            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.cancelBtn} onPress={() => setModalConfirmarCancelarEdicion(true)}>
                <Text style={styles.cancelBtnText}>Cancelar</Text>
              </TouchableOpacity>
      ...
      {/* Modales de Confirmación Globales (Agrupados en la raíz JSX limpia) */}
      <ModalConfirmacion
        visible={modalConfirmarCancelarEdicion}
        titulo="¿Cancelar edición?"
        mensaje="Si cancelás ahora, todos los cambios que no hayas guardado en el usuario se perderán."
        textoConfirmar="Sí, cancelar"
        textoCancelar="Seguir editando"
        onConfirm={() => {
          setModalConfirmarCancelarEdicion(false);
          setModalEditar(false);
        }}
        onCancel={() => setModalConfirmarCancelarEdicion(false)}
      />

      <ModalConfirmacion
        visible={usuarioAEliminar !== null}
        titulo={usuarioAEliminar?.rol === 'alumno' ? "¿Eliminar alumno permanentemente?" : "Confirmar eliminación"}
        mensaje={obtenerMensajeEliminacion()}
...
const styles = StyleSheet.create({
  ...
  tabBar: { flexDirection: 'row', backgroundColor: '#FFFFFF', paddingHorizontal: 16, paddingVertical: 10, gap: 8, borderBottomWidth: 1, borderBottomColor: '#F3F4F6' },
  tabBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 8, borderRadius: 10, backgroundColor: '#F3F4F6' },
  tabBtnActive: { backgroundColor: '#E8F5E9' },
  tabBtnText: { fontSize: 14, fontWeight: '600', color: '#9CA3AF' },
  tabBtnTextActive: { color: '#0F4A32' },
  card: { backgroundColor: '#FFFFFF', borderRadius: 12, padding: 14, marginBottom: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', elevation: 1, borderLeftWidth: 3, borderLeftColor: '#25B471' },
  cardInfo: { flex: 1 },
  ...
  editBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#E8F5E9', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 },
  editBtnText: { fontSize: 13, fontWeight: '600', color: '#0F4A32' },
  deleteBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#FEF2F2', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 },
  ...
  cursadaCard: { backgroundColor: '#FFFFFF', borderRadius: 12, marginBottom: 12, overflow: 'hidden', elevation: 1, borderLeftWidth: 3, borderLeftColor: '#25B471' },
  cursadaCardHeader: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14 },
  cursadaIconBg: { width: 40, height: 40, borderRadius: 10, backgroundColor: '#E8F5E9', justifyContent: 'center', alignItems: 'center' },
  cursadaTitulo: { fontSize: 15, fontWeight: '700', color: '#11181C' },
  cursadaSubtitulo: { fontSize: 12, color: '#9CA3AF', marginTop: 2 },
  cursadaPanel: { borderTopWidth: 1, borderTopColor: '#F3F4F6', padding: 14, gap: 4 },
  panelLabel: { fontSize: 11, fontWeight: '700', color: '#9CA3AF', textTransform: 'uppercase', marginTop: 8, marginBottom: 6 },
  ...
  regenerarBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#FEF2F2', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 },
  ...
  asignarBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#E8F5E9', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8 },
  asignarBtnText: { fontSize: 13, fontWeight: '600', color: '#0F4A32' },
  inscriptoRow: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#F9F9F9' },
  eliminarBtn: { flex: 1, backgroundColor: '#DC2626', borderRadius: 12, paddingVertical: 4, alignItems: 'center' },
  ...
  modalContent: { backgroundColor: '#FFFFFF', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 24, paddingBottom: 36 },
  ...
  input: { backgroundColor: '#F9F9F9', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, fontSize: 15, color: '#11181C' },
  rolBtn: { flex: 1, paddingVertical: 10, borderRadius: 10, borderWidth: 1.5, borderColor: '#E5E7EB', alignItems: 'center', backgroundColor: '#FFFFFF' },
  ...
  cancelBtn: { flex: 1, backgroundColor: '#FFFFFF', borderRadius: 12, borderWidth: 1.5, borderColor: '#E5E7EB', paddingVertical: 14, alignItems: 'center' },
  cancelBtnText: { color: '#6B7280', fontSize: 15, fontWeight: '700' },
  saveBtn: { flex: 1, backgroundColor: '#25B471', borderRadius: 12, paddingVertical: 14, alignItems: 'center' },
